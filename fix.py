...
class PrepareWorker(QObject):
 ...
 def _find_java(self)->str|None:
  print("Attempting to auto-detect Java...")
  system = platform.system()
  # Assuming Windows as per request
  exe_name = "javaw.exe"
  alt_exe_name = "java.exe"
  search_paths = []
  home_dir = Path.home()
  checked_executables = set()

  # 1. Lunar Client JRE (Windows)
  lunar_jre_base = home_dir / ".lunarclient" / "jre"
  if lunar_jre_base.is_dir():
    # Use glob to find the specific Zulu 17 bin directory, account for intermediate hash dir
    # Sort ensures we likely get the latest if multiple exist (though usually just one)
    lunar_zulu_bins = sorted(lunar_jre_base.glob('**/zulu17*/bin'), reverse=True)
    if lunar_zulu_bins:
      first_lunar_bin = lunar_zulu_bins[0]
      potentials = [first_lunar_bin / exe_name]
      if alt_exe_name: potentials.append(first_lunar_bin / alt_exe_name)
      for exe_path in potentials:
        if exe_path not in checked_executables and exe_path.is_file():
          checked_executables.add(exe_path)
          try:
            res = subprocess.run([str(exe_path),'-version'], capture_output=True, text=True, timeout=3, check=False, encoding='utf-8', errors='ignore')
            ver = res.stdout + res.stderr
            if " 17." in ver or ' version "17.' in ver:
              print(f"  Found suitable Lunar Client Java 17: {exe_path}")
              return str(exe_path)
          except Exception: # Broad except to avoid stopping on permission/timeout etc.
            pass # If check fails, move on

  # 2. Minecraft Runtimes (Windows - standard AppData and UWP)
  mc_runtime_bases = []
  localappdata = os.getenv('LOCALAPPDATA')
  appdata = os.getenv('APPDATA')
  if appdata:
    mc_runtime_bases.append(Path(appdata) / ".minecraft" / "runtime")
  if localappdata:
    uwp_base = Path(localappdata) / "Packages"
    # Using try-except for potential permission issues accessing Packages
    try:
      mc_uwp_pkgs = list(uwp_base.glob("Microsoft.MinecraftUWP*"))
      if mc_uwp_pkgs:
        # Check if the potential path exists before adding
        uwp_runtime_path = mc_uwp_pkgs[0] / "LocalCache" / "minecraft-launcher" / "runtime"
        if uwp_runtime_path.exists():
          mc_runtime_bases.append(uwp_runtime_path)
    except OSError:
      pass # Ignore permission errors listing packages

  for mc_base in mc_runtime_bases:
    if mc_base.is_dir():
      # Glob specifically for java-runtime* ending in 17* and get bin dir
      # Prioritize non-ARM runtimes if distinction exists in name (heuristic)
      java17_bins = sorted(mc_base.glob("**/java-runtime*-17*/bin"), key=lambda p: "arm" in str(p).lower(), reverse=False)
      for bin_path in java17_bins:
        if not bin_path.is_dir(): continue
        potentials = [bin_path/exe_name]
        if alt_exe_name: potentials.append(bin_path / alt_exe_name)
        found_mc_java = False
        for exe_path in potentials:
          if exe_path not in checked_executables and exe_path.is_file():
            checked_executables.add(exe_path)
            try:
              ...
              ver = res.stdout + res.stderr
              if " 17." in ver or ' version "17.' in ver:
                print(f"  Found suitable Minecraft Java 17: {exe_path}")
                return str(exe_path)
            except Exception:
              pass # Ignore check errors
        if found_mc_java: break # Found valid Java 17 in this runtime, move to next base


  # 3. JAVA_HOME env var
  java_home = os.getenv('JAVA_HOME')
  if java_home and (bin_path := Path(java_home)/"bin").is_dir():
    potentials = [bin_path/exe_name]
    if alt_exe_name: potentials.append(bin_path/alt_exe_name)
    for exe_path in potentials:
      if exe_path not in checked_executables and exe_path.is_file():
        ...
        try:
          ...
          ver = res.stdout + res.stderr
          if " 17." in ver or ' version "17.' in ver:
            print(f"  Found suitable JAVA_HOME Java 17: {exe_path}")
            return str(exe_path)
        except Exception:
          pass

  # 4. PATH (javaw or java) using shutil.which
  sys_path = shutil.which(exe_name)
  alt_sys_path = shutil.which(alt_exe_name) if alt_exe_name else None

  path_executables = []
  if sys_path: path_executables.append(Path(sys_path))
  if alt_sys_path and Path(alt_sys_path) not in path_executables: path_executables.append(Path(alt_sys_path))
  ...
  for exe_path in path_executables:
    if exe_path not in checked_executables and exe_path.is_file():
      ...
      try:
        ...
        ver = res.stdout + res.stderr
        if " 17." in ver or ' version "17.' in ver:
          print(f"  Found suitable PATH Java 17: {exe_path}")
          return str(exe_path)
      except Exception:
        pass

  # 5. Last resort: Return first found in PATH even if not validated as 17
  if sys_path and Path(sys_path).is_file():
    print(f"  Using Java from PATH (version not verified as 17): {sys_path}")
    return sys_path
  if alt_sys_path and Path(alt_sys_path).is_file():
    print(f"  Using Java from PATH (version not verified as 17): {alt_sys_path}")
    return alt_sys_path

  print("!!! Error: Could not find any Java installation via common methods.")
  return None
 def run(self):
  try:
